...
# ==================================================
# PROTECT BY ZAROFFC - PANEL PROTECTION SCRIPT
# Base by : ZeroHosting
# Create by : ZarOffc
# Version: 1.1
# ==================================================
    ...
    public function handle(Server $server): void
    {
        ...
        if ($user) {
            if ($user->id !== 1) {
                ...
                if ($ownerId !== $user->id) {
                    throw new DisplayException('Akses ditolak: Anda hanya dapat menghapus server milik Anda sendiri © PROTECT BY ZAROFFC');
                }
            }
...
class UserController extends Controller
{
    ...
    public function delete(Request $request, User $user): RedirectResponse
    {
        ...
        if ($request->user()->id !== 1) {
            throw new DisplayException("❌ Hanya admin ID 1 yang dapat menghapus user lain! © Protected by ZarOffc V1.1");
        }
        // ============================================
    ...
    public function update(UserFormRequest $request, User $user): RedirectResponse
    {
        ...
        foreach ($restrictedFields as $field) {
            if ($request->filled($field) && $request->user()->id !== 1) {
                throw new DisplayException("⚠️ Data hanya bisa diubah oleh admin ID 1. © Protected by ZarOffc V1.1");
            }
        }

        // Cegah turunkan level admin ke user biasa
        if ($user->root_admin && $request->user()->id !== 1) {
            throw new DisplayException("🚫 Tidak dapat menurunkan hak admin pengguna ini. Hanya ID 1 yang memiliki izin. © Protected by ZarOffc V1.1");
        }
        // ====================================================
...
class LocationController extends Controller
{
    ...
    public function index(): View
    {
        ...
        $user = Auth::user();
        if (!$user || $user->id !== 1) {
            abort(403, 'Protection by ZarOffc - Anda tidak Memiliki Akses untuk Melakukan Hal Ini!');
        }
...
class NodeController extends Controller
{
    ...
    public function index(Request $request): View
    {
        ...
        $user = Auth::user();
        if (!$user || $user->id !== 1) {
            abort(403, '🚫 Akses ditolak! Hanya admin ID 1 yang dapat membuka menu Nodes. © Protection by ZarOffc V1.1');
        }
        // ======================================================
...
class NestController extends Controller
{
    ...
    public function index(): View
    {
        ...
        $user = Auth::user();
        if (!$user || $user->id !== 1) {
            abort(403, '🚫 Akses ditolak! Hanya admin utama (ID 1) yang bisa membuka menu Nests. © Protection by ZarOffc V1.1');
        }
...
echo -e "${BLUE}Starting Protection Panel Pterodactyl installation...${NC}"
echo ""
...
echo ""
echo -e "${GREEN}==================================================${NC}"
echo -e "${GREEN}    PROTECTION PANEL INSTALLATION COMPLETE!${NC}"
echo -e "${GREEN}            Created by ZarOffc${NC}"
echo -e "${GREEN}==================================================${NC}"
echo ""
...
echo ""
echo -e "${BLUE}Your Pterodactyl panel is now protected!${NC}"
echo ""
